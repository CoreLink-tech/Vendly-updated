          ...
          <div key={s} className="flex items-center flex-1 last:flex-none">
            <div className="flex flex-col items-center gap-1.5">
              <span
                ...
                style={{ backgroundColor: i < 3 ? landing.green : landing.line }}
              />
              <p className="text-[8px] text-center leading-tight w-14" style={{ color: i < 3 ? landing.ink : landing.cocoa }}>{s}</p>
            </div>
            {i < stages.length - 1 && (
              <span className="h-px flex-1 -mt-4" style={{ backgroundColor: i < 2 ? landing.green : landing.line }} />
            )}
          </div>
        ))}
      </div>
      <div className="mt-4 pt-3 flex items-center justify-between" style={{ borderTop: `1px solid ${landing.line}` }}>
        <p className="text-[9px]" style={{ color: landing.cocoa }}>Route: Ikeja → Yaba</p>
        <p className="text-[10px] font-semibold" style={{ color: landing.ink }}>₦1,500 delivery</p>
      </div>
    </Frame>
  );
}
  ...
  return (
    <Frame label="Checkout">
      <div className="flex gap-2 mb-3">
        <div className="flex-1 rounded-xl p-2.5" style={{ backgroundColor: landing.green }}>
          <p className="text-[9px] font-semibold" style={{ color: landing.paper }}>Pay now</p>
          <p className="text-[8px] mt-1" style={{ color: 'rgba(255,254,251,0.75)' }}>Card, bank, transfer</p>
        </div>
        <div className="flex-1 rounded-xl p-2.5" style={{ border: `1px solid ${landing.line}` }}>
          <p className="text-[9px] font-semibold" style={{ color: landing.ink }}>On delivery</p>
          <p className="text-[8px] mt-1" style={{ color: landing.cocoa }}>Pay when it arrives</p>
        </div>
      </div>
      <div className="flex items-center justify-between text-[10px]" style={{ color: landing.cocoa }}>
        <span>You keep</span>
        <span className="font-semibold" style={{ color: landing.ink }}>100% — no commission</span>
      </div>
    </Frame>
  );
}
  ...
  return (
    <Frame label="This month">
      <div className="flex items-end justify-between mb-3">
        <div>
          <p className="text-[9px]" style={{ color: landing.cocoa }}>Revenue</p>
          <p className="text-[18px] font-semibold" style={{ ...displayFont, color: landing.ink }}>₦1.24m</p>
        </div>
        <span className="text-[9px] font-medium px-2 py-1 rounded-full" style={{ backgroundColor: landing.orangeSoft, color: landing.orange }}>+18%</span>
      </div>
      <div className="flex items-end gap-1.5 h-14">
        {[35, 55, 40, 70, 60, 85, 50, 95, 65, 75, 90, 100].map((h, i) => (
          <span key={i} className="flex-1 rounded-[2px]" style={{ height: `${h}%`, backgroundColor: i === 11 ? landing.green : landing.greenSoft }} />
        ))}
      </div>
    </Frame>
  );
  ...
  return (
    <Frame label="Customers · 213">
      <div className="space-y-2.5">
        {rows.map((r) => (
          <div key={r.name} className="flex items-center gap-2.5">
            <div className="w-7 h-7 rounded-full shrink-0 flex items-center justify-center text-[9px] font-semibold" style={{ backgroundColor: landing.greenSoft, color: landing.green }}>
              {r.name[0]}
            </div>
            <div className="min-w-0 flex-1">
              <p className="text-[11px] font-medium truncate" style={{ color: landing.ink }}>{r.name}</p>
              <p className="text-[9px]" style={{ color: landing.cocoa }}>{r.orders}</p>
            </div>
            <p className="text-[10px] font-semibold shrink-0" style={{ color: landing.ink }}>{r.spent}</p>
          </div>
        ))}
      </div>
    </Frame>
