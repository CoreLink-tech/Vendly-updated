  ...
  return (
    <div>
      ...
      <div className="rounded-xl border overflow-hidden" style={{ backgroundColor: theme.surface, borderColor: theme.line }}>
        {loading ? (
          ...
          <div className="text-center py-12">
            <p className="text-sm" style={{ color: theme.faint }}>No reports right now.</p>
            ...
            {reports.map((r) => (
              ...
              >
                ...
                <div className="hidden md:flex flex-col items-end gap-0.5 shrink-0">
                  <span className="text-xs" style={{ color: theme.muted }}>{fmtDate(r.createdAt)}</span>
                  <span className="text-[10px]" style={{ color: theme.orange }}>{timeLeftLabel(r.createdAt)}</span>
                </div>
              </div>
            ))}
          </div>
      ...
      {selected && (
        ...
        >
          <div className="w-full max-w-md rounded-xl border" style={{ backgroundColor: theme.surface, borderColor: theme.line }}>
            <div className="flex items-center justify-between px-6 py-4 border-b" style={{ borderColor: theme.line }}>
              <p className="text-sm font-semibold" style={{ color: theme.ink }}>Report</p>
            ...
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <p className="text-xs" style={{ color: theme.muted }}>Name</p>
                  <p className="text-sm font-medium mt-0.5" style={{ color: theme.ink }}>{selected.customerName}</p>
                </div>
                <div>
                  <p className="text-xs" style={{ color: theme.muted }}>Phone</p>
                  <p className="text-sm font-medium mt-0.5" style={{ color: theme.ink }}>{selected.customerPhone}</p>
                </div>
                <div>
                  <p className="text-xs" style={{ color: theme.muted }}>Submitted</p>
                  <p className="text-sm font-medium mt-0.5" style={{ color: theme.ink }}>{fmtDate(selected.createdAt)}</p>
                </div>
                <div>
                  <p className="text-xs" style={{ color: theme.muted }}>Status</p>
                  <p className="text-sm font-medium mt-0.5" style={{ color: theme.orange }}>{timeLeftLabel(selected.createdAt)}</p>
                </div>
              </div>

              <div>
                <p className="text-xs mb-1.5" style={{ color: theme.muted }}>Message</p>
                <p className="text-sm leading-relaxed p-3 rounded-lg" style={{ backgroundColor: theme.bg, color: theme.ink }}>
                  {selected.message}
                </p>
              </div>
