...
export default function LogisticsRoutesPage() {
  ...
  return (
    <div>
      <div className="mb-8">
        <h1 className="text-2xl font-semibold tracking-tight" style={{ color: theme.ink }}>Logistics Routes</h1>
        <p className="text-sm mt-1" style={{ color: theme.muted }}>Set delivery prices between states. Vendors using Vendly logistics will show these fees to buyers.</p>
      </div>
      ...
      <div className="rounded-xl border p-6 mb-6" style={{ backgroundColor: theme.surface, borderColor: theme.line }}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
          <label className="flex flex-col gap-1.5 text-xs font-medium" style={{ color: theme.muted }}>
            ...
            <select
              ...
              style={{ backgroundColor: theme.bg, borderColor: theme.line, color: form.fromState ? theme.ink : '#555' }}
            >
          ...
          <label className="flex flex-col gap-1.5 text-xs font-medium" style={{ color: theme.muted }}>
            ...
            <select
              ...
              className="rounded-lg border px-3 py-2.5 text-sm outline-none"
              style={{ backgroundColor: theme.bg, borderColor: theme.line, color: form.toState ? theme.ink : '#555' }}
            >
              <option value="">Select state</option>
              {NIGERIAN_STATES.map((s) => <option key={s} value={s}>{s}</option>)}
            </select>
        ...
        {message && (
          <p className="text-xs mb-3" style={{ color: message.type === 'success' ? theme.green : '#ef4444' }}>{message.text}</p>
        )}
        <button
            ...
            {filtered.map((route) => (
              <div key={route.id} className="flex items-center justify-between px-5 py-3.5">
                <div className="flex items-center gap-3">
                  <span className="text-sm" style={{ color: theme.ink }}>{route.fromState}</span>
                  <span style={{ color: '#555' }}>→</span>
                  <span className="text-sm" style={{ color: theme.ink }}>{route.toState}</span>
                </div>
                <div className="flex items-center gap-4">
                  <span className="text-sm font-semibold" style={{ color: theme.green }}>₦{Number(route.price).toLocaleString()}</span>
                  <button
